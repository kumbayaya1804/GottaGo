...
# Antigravity Review Request — Phase 4: GPS Service & Submission

You are Antigravity, senior architect and lead systems auditor for Gotta Go. Focus per your role: PostGIS correctness, RLS placement, trust/confidence math, data integrity, and emergency-user availability. Apply the 60-second user advocacy check. Output your verdict in the standard format from ANTIGRAVITY.md (VERDICT / Reviewed Queue / Issues / Concerns / Verification / Runtime Boundary Check / Approved).
...
Antigravity-specific checks to repeat:

1. Door-code data integrity:
   ...
   - confirm the migration does not weaken `deleted_at`, `shadowban_status`, or `suppressed_at` filtering
...
2. Submit-flow event ordering:
   - `submit.tsx` gates Step 2 with `await trigger(['hours', 'accessCode', 'timingTip'])`
   - final submit uses a synchronous `submittingRef` guard reset in mutation `onSettled`
   - `submit.test.tsx` covers overlong timing-tip blocking and double-press single-submit behavior
