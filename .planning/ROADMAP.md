...
**Goal**: A second independent user can GPS-verify a location, triggering the trust engine. The creator's implicit claim (which counts even when the creator is currently shadowbanned — see D-69) plus one currently-eligible independent verifier publish the location; a currently-shadowbanned creator's claim still counts but the published location inherits shadowban_status=true, is suppressed from public search, and earns the creator no trust credit (D-69). Trust score and confidence score update incrementally.
**Depends on**: Phase 4
