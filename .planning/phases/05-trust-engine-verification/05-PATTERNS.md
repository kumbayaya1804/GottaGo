...
**The concurrency-safe deciding-verifier publish + trust appends — SINGLE SOURCE OF TRUTH is `05-02-PLAN.md` Task 3 steps 3 / 5 / 8 / 9. Do NOT duplicate that transaction’s SQL here.**
...
- **Decision-time eligibility, lock-protected on ALL THREE sides — creator, current caller, and historical verifiers (D-52 / D-69):** the qualifying count is `1` (creator's implicit claim) `+ distinct non-creator verifiers`, where each counted verifier satisfies BOTH the immutable recorded `weight > 0` AND a live `shadowban_status is not true` re-check. That live shadowban read — for the counted verifiers, for the creator, **and for the CURRENT CALLER's own row** (whose shadowban status also gates its OWN weight computation and trust effects, not just the later count) — is taken under a genuine `FOR NO KEY UPDATE` row lock, never a plain (MVCC-snapshot) `SELECT`, lock-free JOIN, or a `FOR SHARE` lock that would later need an upgrade. All these user-row locks are acquired in ONE deterministic global order, in ONE pass, BEFORE any weight is computed or any trust effect applied: the `submissions` row `FOR UPDATE` first, then every involved `public.users` row (creator + current caller + counted historical verifiers) `FOR NO KEY UPDATE` in ascending `users.id` order — acquired at that single strength from the start so no row is ever locked once and upgraded later (a later upgrade on an overlapping set is what reopens a deadlock). Any future admin/moderation shadowban write path MUST touch user rows in that same ascending-`users.id` order to stay deadlock-free.
- **Conditional trust appends — neither is unconditional (D-48 / D-49 / D-50 / D-69):** on publish, the creator’s `published_contribution` append happens ONLY when that lock-read creator `shadowban_status` is NOT true — a currently-shadowbanned creator’s suppressed publish mints NO creator trust. The deciding verifier’s `verification_given_nonzero` append + `trust_multiplier` ramp happen ONLY for a nonzero-weight (non-shadowban) event, computed from the SAME lock-protected read of the caller's own row.
- **Shadowban inheritance (D-69):** the new `locations` row’s `shadowban_status` is set from that same lock-read creator value, reusing the Phase 3 `and shadowban_status = false` public-search suppression — the creator’s claim still counts toward the threshold, but a shadowbanned creator’s location publishes suppressed.
