...
- **D-69:** A submission whose creator is CURRENTLY shadowbanned at the publish decision still counts the creator's implicit claim toward the publish threshold (the verifier's real contribution is preserved and still counts), and the resulting `locations` row inherits `shadowban_status=true` from the creator (reusing the exact suppression mechanism Phase 3's public search RPCs already apply), but the creator receives NO `published_contribution` trust_events credit for a submission that is immediately suppressed. A non-shadowbanned creator's publish behaves exactly as originally specified. (Confirmed by the round-6 plan review, refined in round 9; the creator's CURRENT shadowban_status must be read under a genuine `FOR NO KEY UPDATE` lock on the creator's `users` row — not a plain SELECT, not a `FOR SHARE` lock needing a later upgrade, and not covered by the unrelated `submissions` row's `FOR UPDATE` from a different table — acquired in the SAME single ascending-`users.id` pass that also locks the current caller's own row and every historical counted verifier's row, to serialize correctly against a concurrent shadowban action and to avoid a deadlock against a reciprocal call on a different submission.)
