...
| **Full suite command** | `supabase test db --local` on a Docker-capable environment + `cd app && npm test`, EXCEPT `phase5_verify_publish.test.sql` (contains HISTORICAL-VERIFIER-SHADOWBAN-RACE, which commits a real global `app_config` mutation) and `phase5_discovery_cooldown_race.test.sql` (uses dblink for a genuine two-session race and commits real fixture rows for the second session to see) — both files MUST run via `node supabase/scripts/run-isolated-db-suite.js <file>`, which provisions a disposable, single-use instance and ALWAYS ATTEMPTS teardown after any start attempt (not only after success); a teardown that itself fails is reported as a hard failure with the workdir preserved for manual recovery — it is not silently downgraded to a warning, but it is also not an unconditional guarantee that no external resource can ever remain. A shared/persistent Docker environment is not an acceptable substitute for either file |
...
| D-36 | discovery + verification cooldowns are server-enforced and rejected attempts persist the timestamp; the discovery claim is proven atomic under real two-session concurrency, not just sequential reads | pgTAP | `supabase test db --local supabase/tests/phase5_discovery.test.sql` PLUS `node supabase/scripts/run-isolated-db-suite.js supabase/tests/phase5_discovery_cooldown_race.test.sql supabase/tests/phase5_verify_publish.test.sql` (REQUIRED for both — the discovery race file commits real cross-session fixtures, never run via plain `supabase test db`) | ❌ Wave 0 | ⬜ pending |
...
- [ ] `supabase/tests/phase5_discovery.test.sql` — 500m radius, exclusions (own + already-verified), result cap, no submitter-identity leak, denied-call-still-advances-claim
- [ ] `supabase/tests/phase5_discovery_cooldown_race.test.sql` — two-session dblink race proving the discovery rate-limit claim is atomic (REQUIRED via isolated runner, commits real cross-session fixtures)
- [ ] `supabase/tests/phase5_verify_publish.test.sql` — concurrency: CREATOR-SHADOWBAN-RACE, HISTORICAL-VERIFIER-SHADOWBAN-RACE (isolated threshold-3 fixture), CURRENT-CALLER-SHADOWBAN-RACE (two-session each), and a RECIPROCAL-USER lock-order deadlock test across two submissions with overlapping creator/caller sets; plus shadowban-zero, atomicity, rollback, trust delta sign AND trust_score application (SC6)
